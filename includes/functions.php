...
function login($email, $password, $mysqli) {
    ...
    if ($stmt = $mysqli->prepare("SELECT id, username, realname, password, salt 
        FROM members
       WHERE email = ?
        LIMIT 1")) {
        ...
        $stmt->bind_result($user_id, $username, $realname, $db_password, $salt);
        $stmt->fetch();
...
function changepw($username, $password, $mysqli) {
    // Aktuelles Passwort überprüfen
    if ($stmt = $mysqli->prepare("SELECT id, password, salt FROM members WHERE username = ? LIMIT 1")) 
        {
        $stmt->bind_param('s', $username);  // Bind "$actualun" to parameter.
        $stmt->execute();    // Führe die vorbereitete Anfrage aus.
        $stmt->store_result();
